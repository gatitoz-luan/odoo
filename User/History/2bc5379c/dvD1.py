...
class SaleOrderLine(models.Model):
    ...
    def _compute_rental_values(self):
        for line in self:
            ...
            # Calcula a duração da locação apenas para produtos do tipo 'product'
            if rental_start and rental_end and line.product_id.type == 'product':
                ...
                rental_duration = 1  # Assume a duração como 1 para não alterar o cálculo para outros tipos

            # Ajusta o preço unitário e o desconto se necessário
            price_unit = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
            # Calcula o subtotal. Se o produto for 'product', multiplica pela duração. Caso contrário, usa a quantidade diretamente.
            if line.product_id.type == 'product':
                amount_untaxed = price_unit * line.product_uom_qty * rental_duration
            else:
                amount_untaxed = price_unit * line.product_uom_qty

            # Calcula os impostos
            if line.tax_id:
                taxes = line.tax_id.compute_all(price_unit, line.order_id.currency_id, line.product_uom_qty * (rental_duration if line.product_id.type == 'product' else 1), product=line.product_id, partner=line.order_id.partner_id)
            ...
            line.update({
                'rental_duration': rental_duration if line.product_id.type == 'product' else 0,  # Guarda a duração somente para produtos do tipo 'product'
    ...
    @api.depends('state', 'product_id', 'untaxed_amount_invoiced', 'qty_delivered', 'product_uom_qty', 'price_unit','order_id.rental_start_date', 'order_id.rental_end_date', 'product_uom_qty', 'discount', 'price_unit', 'tax_id')
    def _compute_untaxed_amount_to_invoice(self):
        ...
        for line in self:
            ...
            if line.state == 'sale':
                ...
                rental_end_date = fields.Date.from_string(rental_end)
                rental_duration = (rental_end_date - rental_start_date).days + 1
                price_subtotal = price_reduce * uom_qty_to_consider * rental_duration
