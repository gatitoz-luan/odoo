...
class SaleOrderLine(models.Model):
    ...
    def _compute_rental_values(self):
        for line in self:
            ...
            # Define a duração da locação e o multiplicador de quantidade base
            if rental_start and rental_end and line.product_id.type == 'product':
                ...
                quantity_multiplier = rental_duration
            else:
                rental_duration = 0
                quantity_multiplier = 1 if line.product_id.type == 'product' else 0

            # Ajusta o preço unitário e o desconto se necessário
            price_unit = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
            # Calcula o subtotal com base no preço unitário, quantidade e multiplicador de quantidade
            amount_untaxed = price_unit * line.product_uom_qty * quantity_multiplier

            # Calcula os impostos
            if line.tax_id:
                taxes = line.tax_id.compute_all(price_unit, line.order_id.currency_id, line.product_uom_qty * quantity_multiplier, product=line.product_id, partner=line.order_id.partner_id)
