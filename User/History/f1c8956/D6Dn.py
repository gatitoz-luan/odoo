...
def _create_stock_out_move_for_stockable_products(self, stock_in_move):
    """Cria um movimento de estoque de saída para um produto estocável baseado no movimento de entrada."""
    StockMove = self.env['stock.move']
    # Acessa o pedido de venda relacionado ao movimento de entrada
    sale_order = stock_in_move.sale_line_id.order_id if stock_in_move.sale_line_id else None
    if not sale_order:
        return  # Retorna se não encontrar um pedido de venda associado

    rental_start_date = fields.Datetime.from_string(sale_order.rental_start_date) if sale_order.rental_start_date else None
    cleaning_time = timedelta(days=stock_in_move.product_id.cleaning_time) if stock_in_move.product_id.cleaning_time else timedelta(0)

    # Calcula a nova data de saída como a data de início do aluguel mais o tempo de limpeza
    stock_out_date = rental_start_date + cleaning_time if rental_start_date else stock_in_move.date

    stock_out_move_vals = {
        'name': stock_in_move.name + ' Out',
        'product_id': stock_in_move.product_id.id,
        'product_uom_qty': stock_in_move.product_uom_qty,
        'product_uom': stock_in_move.product_uom.id,
        'location_id': stock_in_move.location_dest_id.id,
        'location_dest_id': stock_in_move.location_id.id,
        'picking_type_id': stock_in_move.picking_type_id.return_picking_type_id.id or stock_in_move.picking_type_id.id,
        'date': stock_out_date,  # Usa a data de início do aluguel mais o tempo de limpeza
        'state': 'confirmed',  # ou qualquer outro estado desejado
    }
    # Cria o movimento de estoque de saída
    stock_out_move = StockMove.create(stock_out_move_vals)
    stock_out_move._action_confirm()
    stock_out_move._action_assign()
    stock_out_move._action_done()
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        # Novo: Verifica se o tipo do produto é 'product' (estocável)
                if move.product_id.type == 'product':
            # Cria um movimento de estoque de saída para produtos estocáveis
                    self._create_stock_out_move_for_stockable_products(move)
