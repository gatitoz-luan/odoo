...
Please change the quantity done or the rounding precision of your unit of measure.""",
    ...
    @api.model
    def get(self, vals_list):
        # Primeiro, criamos o movimento normalmente
        move = super(StockMove, self).create(vals_list)
        # Em seguida, verificamos se o tipo de operação é de saída e criamos um movimento de entrada correspondente
        if move.location_id.usage == 'customer' and move.location_dest_id.usage == 'supplier':
            self._create_corresponding_in_move(move)

        return move

    def create(self, vals_list):
        # Garante que vals seja uma lista de dicionários
        if isinstance(vals_list, dict):  # Se apenas um dicionário for passado, converta para uma lista
            vals_list = [vals_list]

        for vals in vals_list:
            # Correção: Verifica se 'vals' é um dicionário
            if isinstance(vals, dict) and (vals.get('quantity') or vals.get('move_line_ids')) and 'lot_ids' in vals:
                vals.pop('lot_ids')
        
        return super().create(vals_list)

    def _create_corresponding_in_move(self, move):
        # Encontrar o tipo de operação de entrada correspondente
        ...
        # Criar valores para o novo movimento de entrada
        in_move_vals = {
            'name': move.name,
        ...
        # Criar o movimento de entrada correspondente
        self.create(in_move_vals)
