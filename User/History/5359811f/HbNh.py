...
class SaleOrder(models.Model):
    ...
    def _onchange_rental_start_date(self):
        if self.rental_start_date:
            ...
            self.date_order = self.rental_start_date
    ...
    def _compute_amounts(self):
        ...
        for order in self:
            ...
            amount_untaxed = sum(line.price_subtotal for line in order_lines)  # Multiplicando cada subtotal pela quantidade de dias
            amount_tax = sum(line.price_tax  for line in order_lines)  # Multiplicando cada valor de imposto pela quantidade de dias
    ...
    @api.depends('order_line.price_total', 'order_line.price_subtotal','rental_start_date', 'rental_end_date')
    def _amount_all(self):
        ...
        for order in self:
            ...
            order.update({
                'amount_untaxed': amount_untaxed*delta_days,
                'amount_tax': amount_tax,
                'amount_total': amount_untaxed + amount_tax*delta_days,
            })
    #=== ACTION METHODS ===#
    ...
    def create_stock_picking_for_return(self):
        for order in self:
            # Crie aqui a lógica para criar o pedido de entrada no estoque
            ...
            picking_data = {
                ...
                'origin': order.name,
                'move_lines': [],
                'scheduled_date': order.rental_end_date,  # Use a data de retorno aqui
            }

            for line in order.order_line:
                product_qty = line.product_uom_qty
                picking_data['move_lines'].append((0, 0, {
                    'name': line.name,
            ...
            picking.action_confirm()  # Você pode querer ajustar isso dependendo da configuração do seu workflow
    ...
    def action_confirm(self):
        ...
        self.create_stock_picking_for_return()
