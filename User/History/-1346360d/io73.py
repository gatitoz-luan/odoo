        ...
        for line in self:
            try:
                if line.state == 'sale' and line.product_id.type in ('consu', 'product'):
                    ...
                    # Validação do rental_end_date
                    if line.order_id.rental_end_date and line.order_id.rental_end_date > date_planned:
                        date_planned = line.order_id.rental_end_date
                    ...
                    self.env['product.replenish'].create(replenish_vals).launch_replenishment()
            except Exception as e:
                _logger.error('Falha ao lançar regras de abastecimento para a linha de pedido de venda %s: %s', line.id, str(e))
                # Dependendo do caso, você pode querer lançar a exceção novamente ou continuar
                raise e
