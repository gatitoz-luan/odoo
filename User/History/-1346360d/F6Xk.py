    ...
    def _action_launch_stock_rule(self, previous_product_uom_qty=False):
        ...
        for line in self:
            if line.state == 'sale' and line.product_id.type in ('consu', 'product'):
                date_planned = line.order_id.rental_end_d# Verifica explicitamente se rental_end_date é uma data válida
if line.order_id.rental_end_date and isinstance(line.order_id.rental_end_date, datetime):
    date_planned = line.order_id.rental_end_date
else:
    date_planned = fields.Datetime.now()ate if line.order_id.rental_end_date else fields.Datetime.now()
