...
class SaleOrderLine(models.Model):
    ...
    def _action_launch_stock_rule(self, previous_product_uom_qty=False):
        ...
        for line in self:
            try:
                if line.state == 'sale' and line.product_id.type in ('consu', 'product'):
                    # Inicializa date_planned com a data/hora atual
                    date_planned = fields.Datetime.now()

                    # Validação e ajuste do rental_end_date
                    rental_end_date = line.order_id.rental_end_date
                    if rental_end_date:
                        if isinstance(rental_end_date, datetime):
                            rental_end_date = rental_end_date.date()
                        if rental_end_date > date_planned.date():
                            date_planned = datetime.combine(rental_end_date, datetime.min.time())

                    # Verifica e aplica cleaning_time, se aplicável
                    cleaning_time = getattr(line.product_id, 'cleaning_time', 0)
                    if isinstance(cleaning_time, int) and cleaning_time > 0:
                        date_planned += timedelta(days=cleaning_time)

                    # Certifique-se de obter o product_uom_id do produto
                    product_uom_id = line.product_id.uom_id.id

                    # Criação do registro product.replenish com os valores corretos
                    self.env['product.replenish'].create({
                        'product_id': line.product_id.id,
                        'product_tmpl_id': line.product_id.product_tmpl_id.id,
                        'quantity': line.product_uom_qty,
                        'date_planned': date_planned,
                        'product_has_variants': line.product_id.product_tmpl_id.product_variant_count > 1,
                        'product_uom_id': product_uom_id,
                        'company_id': line.order_id.company_id.id,
                        'warehouse_id': line.order_id.warehouse_id.id,
                    }).launch_replenishment()
            except Exception as e:
                logging.Logger.error('Falha ao lançar regras de abastecimento para a linha de pedido de venda %s: %s', line.id, e)
                # Dependendo do caso, você pode querer tratar a exceção de forma diferente
