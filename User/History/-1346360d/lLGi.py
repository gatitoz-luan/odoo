    ...
    def _action_launch_stock_rule(self, previous_product_uom_qty=False):
        for line in self:
            if line.state == 'sale' and line.product_id.type in ('consu', 'product'):
                ...
                cleaning_time = line.product_id.cleaning_time if hasattr(line.product_id, 'cleaning_time') else 0

                # Calcula uma nova data planejada considerando o cleaning_time
                if cleaning_time:
                    date_planned = fields.Datetime.add(date_planned, days=cleaning_time)
                ...
                self.env['product.replenish'].create({
                    ...
                    'product_uom_id': product_uom_id,  # Defina o product_uom_id aqui
